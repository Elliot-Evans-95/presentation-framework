export class ShellElement {
    ...
    constructor(id: string) {
        this._HTMLElement =
            ...
            <HTMLElement>document.createElement(id)
    }
    ...
    addEventListener(listener: string): Promise<void> {
        return new Promise((resolve) =>
            this._HTMLElement.addEventListener(listener, () => resolve())
        )
    }

    addComponent(component: HTMLElement, insertPosition?: InsertPosition) {
        if (!this._HTMLElement.insertAdjacentElement || !insertPosition) return
